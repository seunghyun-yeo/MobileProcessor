...
typedef struct
{
	bool invalid, taken, in;
	unsigned int pc,nextpc,BTBindex,inst;
}ifid;
...
typedef struct
{
	bool memtoreg, regwrite,memwrite,memread,branch,regdst,alusrc,invalid,in,taken;
        int rsread,rtread,signextimm,wbdest,BTBindex;
        unsigned int opcode,shamt,function,nextpc,inst,rs,rt;
}idex;
...
//	print=(bool)tprint;
	...
	fclose(stream);
	if(!(strcmp(path,"fib.bin")))memory[4]=0x2402001e;
...
void fetch()
{
	...
	if(in)
	{
		if(gbh==2||gbh==3) taken=1;
		else taken=0;
		if(taken) pc=BTB_Dp[x][1];
	}
	else taken=0;
	ifidout.BTBindex=x;
	ifidout.in=in;
...
void decode()
{
	...
	if(idexout.branch)
	{
		...
		if(!ifidin.in)
		{
			BTB_Dp[ifidin.BTBindex][0]=ifidin.pc;
			BTB_Dp[ifidin.BTBindex][1]=ifidin.nextpc+(idexout.signextimm<<2);
		}
	}
	...
	idexout.rs=rs;
	idexout.rt=rt;
	idexout.in=ifidin.in;
	idexout.taken=ifidin.taken;
	idexout.BTBindex=ifidin.BTBindex;
